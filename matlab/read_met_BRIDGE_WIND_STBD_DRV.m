function STBD_TW = read_met_BRIDGE_WIND_STBD_DRV(metdir,tmin,tmax)
...
loadname = join([metdir 'BRIDGE-WIND-STBD-DRV-Data_' loaddate '-*.Raw'],'');
...
try 
    ...
    STBD_TW.dd = ddnew + 30/(60*1440);
    STBD_TW.TWS=NaN.*ddnew;
    STBD_TW.TWD=NaN.*ddnew; 
    
    for k=1:length(ddnew)
        ii = find(dd>=ddnew(k) & dd<ddnew(k)+ddint);
        if ~isempty(ii)
            STBD_TW.TWS(k)=nanmean(TWS(ii));
            STBD_TW.TWD(k)=average_WD(TWD(ii));
        end
    end
