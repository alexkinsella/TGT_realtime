function [data,test] =  load_getmat(prefix, howmany)
...
[data,test]=fill_field_from_file(data,  prefix, 'u', 'U'              ,'umeas'  );
...
%---------------------------------
function [data,test] = fill_field_from_file(data, prefix, filebase, varname, fieldname)

% data = fill_field_from_file(data, prefix, filebase, varname, fieldname);

filename = sprintf('%s%s.mat', prefix, filebase);
load(filename); test = filename;
